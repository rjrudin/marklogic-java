...
public class DatabasePackageMerger extends AbstractPackageMerger {
    ...
    public String mergeDatabasePackages(List<String> mergePackageFilePaths) {
        try {
            String xml = new String(FileCopyUtils.copyToByteArray(new ClassPathResource("default-content-database.xml")
                    .getInputStream()));
            StreamSource stylesheetSource = new StreamSource(
                    new ClassPathResource("content-database-transform.xsl").getInputStream());
